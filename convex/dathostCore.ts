...
export const uploadFileFromUrl = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      console.log("📥 [UPLOAD] Downloading file from URL:", args.fileUrl);
      ...
      if (downloadUrl.includes("drive.google.com")) {
        ...
        if (match) {
          ...
          console.log("📥 [UPLOAD] Using Google Drive direct URL with confirm bypass");
        }
      }
      ...
      if (!downloadResponse.ok) {
        console.log("⚠️ [UPLOAD] Download failed:", downloadResponse.status);
      ...
      if (contentType.includes("text/html") && fileBuffer.byteLength < 100000) {
        ...
        console.log("⚠️ [UPLOAD] Got HTML response (likely confirmation page):", text.substring(0, 200));
        return { success: false, error: "Google Drive returned HTML confirmation page instead of file" };
      }

      const fileBlob = new Blob([fileBuffer]);

      console.log("📤 [UPLOAD] Uploading to DatHost:", args.remotePath, "size=", fileBuffer.byteLength);
      ...
      if (!uploadResponse.ok) {
        console.log("⚠️ [UPLOAD] Upload failed:", "status=", uploadResponse.status, "response=", responseText);
        return { success: false, error: responseText };
      }

      console.log("✅ [UPLOAD] File uploaded successfully:", args.remotePath);
      return { success: true };
    } catch (error: any) {
...
export const uploadKnifePlugin = internalAction({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      console.log("⚠️ [KNIFE] Error:", error.message);
      return { success: false, error: error.message };
    }
  },
});
...
export const deleteServerFile = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (response.ok) {
        console.log("🧹 [FILES] Deleted:", args.path);
        return;
      }

      const errorText = await response.text();
      console.log("⚠️ [FILES] Delete failed:", args.path, "status=", response.status, "response=", errorText);
    } catch (error: any) {
      console.log("⚠️ [FILES] Delete error:", args.path, error.message);
    }
  },
});
...
export const debugServerSettings = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const text = await response.text();
      if (!response.ok) {
        console.log("⚠️ [DEBUG] GET game-server failed:", "status=", response.status, "response=", text);
        return;
      }
      ...
      try {
        ...
        console.log("⚠️ [DEBUG] GET game-server returned non-JSON:", text);
        return;
      }
      ...
      console.log("🔎 [DEBUG] cs2_settings keys:", keys);
      console.log("🔎 [DEBUG] cs2_settings interesting:", interesting);
      console.log("🔎 [DEBUG] cs2_settings:", cs2);
    } catch (error: any) {
      console.log("⚠️ [DEBUG] Error fetching server:", error.message);
    }
  },
});
...
export const updateServerSettings = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.log(
          "⚠️ [ADDONS] Failed to update settings:",
          "status=", 
          response.status,
          "response=",
          responseText
      ...
      console.log("✅ [ADDONS] Updated settings:", responseText);
    } catch (error: any) {
      console.log("⚠️ [ADDONS] Error updating settings:", error.message);
    }
  },
});
...
export const createServer = internalAction({
  ...
  handler: async (ctx, args) => {
    console.log("🚀 [CREATE SERVER] Starting vanilla CS2 server creation");
    ...
    try {
      ...
      console.log("📤 [CREATE SERVER] Sending create request to DatHost");
      console.log("📍 [CREATE SERVER] Requested location:", args.location || "Auto");
      ...
      if (!createResponse.ok) {
        console.error("❌ [CREATE SERVER] Failed:", createResponse.status);
      ...
      console.log("📥 [CREATE SERVER] Response:", responseText);
      
      let serverData;
      try {
        serverData = JSON.parse(responseText);
      } catch (parseError: any) {
        console.error("❌ [CREATE SERVER] JSON Parse Error:", parseError.message);
        console.error("❌ [CREATE SERVER] Raw response:", responseText);
        throw new Error(`Invalid JSON response: ${responseText.substring(0, 100)}`);
      }
      
      const serverId = String(serverData.id);
      console.log("✅ [CREATE SERVER] Server created:", serverId);
      ...
      console.log("⚙️ [CREATE SERVER] Applying match settings (and MetaMod/CSS flags) via PUT");
      ...
      if (!applyResponse.ok) {
        console.log("⚠️ [CREATE SERVER] PUT settings failed:", "status=", applyResponse.status, "response=", applyText);
      } else {
        console.log("✅ [CREATE SERVER] PUT settings ok:", applyText);
      }
      ...
      if (!startResponse.ok) {
        ...
        console.error("❌ [START SERVER] Failed:", errorText);
        throw new Error(`Failed to start server: ${startResponse.status}`);
      }

      console.log("✅ [START SERVER] Server started");
      ...
      while (attempts < maxAttempts && (!serverIp || !serverPort)) {
        ...
        if (statusResponse.ok) {
          ...
          if (statusData.ip && statusData.ports?.game) {
            ...
            console.log("✅ [BOOT] Server ready:", `${serverIp}:${serverPort}`);
            break;
          }
        }

        attempts++;
        console.log(`⏳ [BOOT] Waiting for server... (${attempts}/${maxAttempts})`);
      }
      ...
      console.log("🎮 [CREATE SERVER] Complete! Server ready for players");
...
export const storeServerInfo = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log("✅ [STORE] Server info saved to database");
  },
});
...
export const initializeServerSettings = internalAction({
  ...
  handler: async (ctx, args) => {
    console.log("⚙️ [INIT] Configuring server settings...");
    ...
    console.log("✅ [INIT] Server configured");
  },
});
...
export const sendRconCommand = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error("❌ [RCON] Failed:", args.command, errorText);
      }
    } catch (error: any) {
...
export const startLogMonitoring = internalAction({
  ...
  handler: async (ctx, args) => {
    console.log("👁️ [MONITOR] Starting log monitoring for whitelist enforcement");
...
export const checkServerLogs = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.attemptCount >= MAX_ATTEMPTS) {
      console.log("⚠️ [MONITOR] Max monitoring time reached");
      return;
    }
    ...
    if (!match || match.state === "FINISHED" || match.state === "CANCELLED") {
      console.log("⚠️ [MONITOR] Match ended, stopping monitoring");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (const line of lines) {
          ...
          if (!normalizedWhitelist.includes(steamId64)) {
            console.log("🚫 [WHITELIST] Unauthorized player detected:", playerName, steamIdRaw, "->", steamId64);
          ...
          console.log("✅ [WHITELIST] Authorized player connected:", playerName, steamIdRaw, "->", steamId64);
...
export const trackConnectedSteamId = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log("✅ [TRACK] connectedSteamIds:", updated, "/", whitelist);

    const expected = whitelist.length;
    if (updated.length >= expected && !match.startSequenceTriggered && match.dathostServerId) {
      console.log("🚀 [TRACK] All whitelisted players connected, starting in 10 seconds");
      await ctx.db.patch(args.matchId, {
...
export const deleteServer = internalAction({
  ...
  handler: async (ctx, args) => {
    console.log("🗑️ [DELETE] Deleting server:", args.serverId);
    ...
    try {
      ...
      console.log("✅ [DELETE] Server deleted successfully");
    } catch (error: any) {
